...
class AnimationModelRegistryTests(unittest.TestCase):
    ...
    def test_viewer_exposes_required_javascript_api(self):
        ...
        self.assertIn("PET_MIN_RENDER_WIDTH = 360", html)
        self.assertIn("PET_MIN_RENDER_HEIGHT = 520", html)
        self.assertIn("autoDensity: true, resolution: renderResolution", html)
        self.assertIn("app.renderer.resolution = renderResolution", html)
        self.assertIn("return {width: Math.max(1, w), height: Math.max(1, h)}", html)
