...
| 快速入口 | 说明 |
|---|---|
| [安装与启动](docs/SETUP.md) | 环境、OCR、Live2D、EXE 与安装包 |
| [配置说明](docs/CONFIG.md) | 模型、网络、Vision、Memory 与 Coding Agent |
| [功能状态](docs/FEATURES.md) | 当前能力、限制与常见问题 |
| [技术架构](docs/TECHNICAL_OVERVIEW.md) | Agent Pipeline、Session、线程和数据流 |
| [开发指南](docs/DEVELOPMENT.md) | 目录约束、扩展方式、注释规范和验证流程 |
| [隐私与安全](docs/PRIVACY_AND_SAFETY.md) | 截图、文件、密钥、Memory 与工具权限 |

## 当前能力

- **桌宠 UI**：透明置顶窗口、拖拽缩放、右键菜单、围栏、帮助与设置页面。
- **动画与行为**：待机、行走、说话、思考、睡眠及数据驱动的摸头、戳脸、庆祝等动作；主动行为默认关闭。
- **Live2D Web 主后端（基础版）**：可扫描仓库外的 `*.model3.json`、选择模型，并在保存配置、重启后作为主桌宠启动；Sprite 仍为默认值，模型、WebEngine、Viewer 或 Runtime 不可用时自动回退 Sprite。显示异常时可右键“切回 Sprite”，或使用 `start_maidie_sprite_safe.bat` / `python main.py --force-sprite` 安全启动，且不会改写配置。
- **聊天与任务展示**：流式聊天气泡、语境化思考短句以及 `CHAT_NATURAL`、`TASK_PROGRESS`、`TASK_STREAM` 三类输出模式；长文本、搜索来源和 Coding Agent 分析进入独立长内容面板。
- **Session 生命周期**：每次请求使用 `request_id`、generation 和事件序号隔离，取消或清空会话后到达的旧回调不会污染新请求或写回历史。
...
- **Memory / Persona**：SQLite 长期记忆、会话内短期上下文、后台记忆提取、对话记录与长期记忆分区清理、人格预设与统一的 Maidie 表达层均已接入基础版本。
...
需要使用隔离的 Live2D WebEngine 预览窗口时，再安装可选依赖：
...
│   ├── session/            # 请求生命周期、OutputEvent 与思考反馈
│   ├── chat/               # 流式文本、分句与气泡控制
...
User Input
  ...
  → Bubble / Long Response Panel / Pet Interaction
```

- Session 负责异步请求、取消隔离和输出顺序，不参与意图判断。
- 工具层负责事实和结构化错误，不直接写用户话术。
- Formatter 负责稳定的结构化摘要，例如 Coding Agent 分析卡片。
- Synthesizer 负责最终自然语言、上下文和人格表达。
- OutputEvent 区分自然对话、Tool 进度与任务流，UI 再选择气泡、来源展示或长内容面板。
