...
| `fence` | 围栏边框显示 |
| `animation` | Sprite / 实验性 Live2D 后端、外部模型根目录和已注册模型 |
...
`backend` 仅接受 `sprite` 或 `live2d_web`，缺失或非法值回退 `sprite`。模型根目录和注册项由“设置 → 动画 / Live2D”维护；打包默认配置不包含任何真实本地路径。第一阶段 `live2d_web` 只提供模型信息预览。
