...
`live2d_pet_scale` 限制为 0.01–1.0，不设置会阻止小窗口 contain 的最小缩放；offset 限制为 ±1000，align 接受 `bottom`、`center`、`top`；`live2d_fit_padding` 限制为 0.5–0.90，默认 0.80。右键“重置 Live2D 显示参数”会恢复上述默认值并立即重新 fit。`--force-sprite` 和 `start_maidie_sprite_safe.bat` 可在配置异常时强制本次使用 Sprite，不修改持久配置。
